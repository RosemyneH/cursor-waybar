...
cJSON *cw_api_get_usage(const char *cookie_header, const char *user_id)
{
	...
	if (cw_https("GET", url, cookie_header, NULL, &code, &body) != 0)
		return NULL;
	if (code != 200 || !body) {
		free(body);
		return NULL;
				     ...
				     int page, int page_size)
{
	...
	snprintf(url, sizeof url, "%s/dashboard/get-filtered-usage-events",
		 CW_API_BASE);

	if (cw_https("POST", url, cookie_header, post, &code, &body) != 0) {
		free(post);
		return NULL;
	}
	free(post);
